 ...
 */
/* Embedded native API implementation derived from the MIT-licensed ESPHome API
 * library by Jonathan Bradshaw (2022); embedded here to avoid runtime dependencies. */
...
private static String encodeMessage(int type, Map tags = [:]) {
    ...
    return bytesToHex(stream.toByteArray())
}
...
private ByteArrayInputStream hexDecode(String hexString) {
    ...
    byte[] payload = hexToBytes(hexString)
...
private static String bytesToHex(byte[] bytes) {
    ...
    for (byte value : bytes) {
        ...
        result.append(digits[unsignedValue >>> 4])
        result.append(digits[unsignedValue & 0x0F])
    }
...
private static byte[] hexToBytes(String value) {
    ...
    byte[] result = new byte[length.intdiv(2)]
...
metadata {
    definition(name: 'ESPHome Power Monitoring Switch', namespace: 'dwinks', author: 'Daniel Winks', singleThreaded: true) {
        capability 'Switch'
        capability 'Refresh'
    ...
    preferences {
        section('ESPHome connection') {
            ...
            input name: 'port', type: 'number', title: 'API port', defaultValue: 6053, required: true
            input name: 'password', type: 'password', title: 'API password', required: false
        }
        section('Entity keys') {
            input name: 'switchEntityKey', type: 'number', title: 'Switch entity key', required: false
            input name: 'powerEntityKey', type: 'number', title: 'Power entity key (optional)', required: false
            input name: 'voltageEntityKey', type: 'number', title: 'Voltage entity key (optional)', required: false
            input name: 'currentEntityKey', type: 'number', title: 'Current entity key (optional)', required: false
            input name: 'energyEntityKey', type: 'number', title: 'Energy entity key (optional)', required: false
        }
        section('Logging') {
            input name: 'logLevel', type: 'enum', title: 'Logging level', options: logLevelOptions(), defaultValue: 'info', submitOnChange: true
        }
    }
}

Map logLevelOptions() {
    return [trace: 'Trace', debug: 'Debug', info: 'Info', warn: 'Warn', error: 'Error', off: 'Off']
}

// Compatibility property used internally by the embedded API implementation.
Boolean getLogEnable() { return settings?.logLevel?.toString() in ['trace', 'debug'] }

void installed() { initialize() }
void updated() { initialize() }
void uninstalled() { closeSocket('driver uninstalled'); unschedule() }
...
void initialize() {
    ...
    if (!settings.port) { device.updateSetting('port', [value: '6053', type: 'number']) }
    unschedule('openSocket')
    openSocket()
}

void refresh() {
    state.requireRefresh = true
    if (device.currentValue('networkStatus') != 'online') { openSocket() }
    else { espHomeListEntitiesRequest() }
}
...
private void sendSwitchCommand(Boolean value) {
    Long key = entityKey(settings.switchEntityKey)
    if (key == null) {
        logWarn('Switch entity key is not configured')
        return
    }
    logInfo("Sending switch ${value ? 'on' : 'off'} command for entity key ${key} (network: ${state.networkStatus ?: 'unknown'})")
    espHomeSwitchCommand([key: key, state: value])
    // The API helper queues commands while offline, but an offline device may
    // not yet have a reconnect scheduled. Open the socket and drain the queue
    // after the native handshake completes.
    if (isOffline()) {
        openSocket()
        runIn(3, 'sendMessageQueue')
    }
}
...
private void updateDeviceMetadata(Map message) {
    ['Device Name': message.name, 'MAC Address': message.macAddress,
     'ESPHome Version': message.espHomeVersion, 'Board Model': message.boardModel,
     'Project Name': message.projectName, 'Project Version': message.projectVersion].each { String key, Object value ->
        if (value != null && value.toString()) { updateDataValue(key, value.toString()) }
    }
    if (message.name && device.displayName != message.name.toString()) { device.setLabel(message.name.toString()) }
    logInfo("Connected to ${message.name ?: device.displayName}")
}

private void cacheEntity(Map message) {
    Long key = entityKey(message.key)
    if (key == null) { return }
    Map entities = (state.entities ?: [:]) as Map
    entities[key.toString()] = [platform: message.platform, name: message.name,
                                objectId: message.objectId, unit: message.unitOfMeasurement,
                                deviceClass: message.deviceClass, disabledByDefault: message.disabledByDefault]
    state.entities = entities
    if (message.platform == 'switch' && !settings.switchEntityKey) {
        device.updateSetting('switchEntityKey', [value: key.toString(), type: 'number'])
        logInfo("Selected switch entity ${message.name ?: key}")
    }
    if (message.platform == 'sensor') {
        String settingName = sensorSettingName(message)
        if (settingName && !settings[settingName]) {
            device.updateSetting(settingName, [value: key.toString(), type: 'number'])
            logInfo("Selected ${settingName} entity ${message.name ?: key}")
        }
    }
}

private String sensorSettingName(Map message) {
    String deviceClass = message.deviceClass?.toString()?.toLowerCase()
    String unit = message.unitOfMeasurement?.toString()?.toLowerCase()
    if (deviceClass == 'power' || unit in ['w', 'kw']) { return 'powerEntityKey' }
    if (deviceClass == 'voltage' || unit == 'v') { return 'voltageEntityKey' }
    if (deviceClass == 'current' || unit == 'a') { return 'currentEntityKey' }
    if (deviceClass == 'energy' || unit in ['wh', 'kwh']) { return 'energyEntityKey' }
    return null
}

private void handleEntityState(Map message) {
    if (message.hasState == false) { return }
    Long key = entityKey(message.key)
    if (key == null) { return }
    if (key == entityKey(settings.switchEntityKey)) {
        sendEvent(name: 'switch', value: message.state ? 'on' : 'off')
    } else if (key == entityKey(settings.powerEntityKey)) {
        sendEvent(name: 'power', value: numberValue(message.state), unit: 'W')
    } else if (key == entityKey(settings.voltageEntityKey)) {
        sendEvent(name: 'voltage', value: numberValue(message.state), unit: 'V')
    } else if (key == entityKey(settings.currentEntityKey)) {
        sendEvent(name: 'amperage', value: numberValue(message.state), unit: 'A')
    } else if (key == entityKey(settings.energyEntityKey)) {
        Map entity = ((state.entities ?: [:]) as Map)[key.toString()] as Map
        BigDecimal energy = numberValue(message.state)
        if (entity?.unit?.toString()?.toLowerCase() == 'wh') { energy = energy.divide(1000G, 3, BigDecimal.ROUND_HALF_UP) }
        sendEvent(name: 'energy', value: energy, unit: 'kWh')
    }
}

private Long entityKey(Object value) {
    try { return value == null || value.toString() == '' ? null : Long.valueOf(value.toString()) }
    catch (Exception ignored) { return null }
}

private BigDecimal numberValue(Object value) {
    try { return new BigDecimal(value.toString()).setScale(3, BigDecimal.ROUND_HALF_UP) }
    catch (Exception ignored) { return 0G }
}
...
private void writeLog(String level, String message) {
    String configured = LOG_LEVELS.contains(settings?.logLevel?.toString()) ? settings.logLevel.toString() : 'info'
    if (configured == 'off' || LOG_LEVELS.indexOf(level) < LOG_LEVELS.indexOf(configured)) { return }
